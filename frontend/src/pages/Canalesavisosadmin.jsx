...
import { AlertCircle, Bell, Calendar, ChevronDown, Filter, MessageSquare, MoreVertical, Plus, Send, Tag, Trash2, Users, X } from 'lucide-react';
...
// ─── Selector con icono (estilo imagen) ──────────────────────────────────────
...
// ─── Barra de filtros ────────────────────────────────────────────────────────
function BarraFiltros({ cuadrillas, filtros, setFiltros, onFiltrar }) {
  return (
    <div style={{ display: 'flex', flexWrap: 'wrap', gap: 16, alignItems: 'flex-end', background: '#fff', border: '1px solid #F1F5F9', borderRadius: 14, padding: '16px 18px', marginBottom: 18 }}>
      <SelectConIcono
        icon={<Users size={15} color="#4F46E5" />}
        label="Cuadrilla"
...
function CanalesAvisosAdmin({ usuario, onLogout }) {
  ...
  const [filtros,     setFiltros]     = useState({ cuadrilla: 'todas', estado: 'todos', desde: '', hasta: '' });
  const [filtrosAplicados, setFiltrosAplicados] = useState({ cuadrilla: 'todas', estado: 'todos', desde: '', hasta: '' });
  ...
  const avisosFiltrados = useMemo(() => {
    return avisos.filter((a) => {
      if (filtrosAplicados.cuadrilla !== 'todas' && String(a.id_cuadrilla) !== String(filtrosAplicados.cuadrilla)) return false;
  ...
  const hayFiltrosActivos = filtrosAplicados.cuadrilla !== 'todas' || filtrosAplicados.estado !== 'todos' || filtrosAplicados.desde || filtrosAplicados.hasta;
